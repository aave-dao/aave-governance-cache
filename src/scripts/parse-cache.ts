...
export function getVotingMachineAddress(
  ...
  if (!addresses) {
    // Testnet chains (sepolia, fuji) don't have migrated VMs —
    // use the address from appConfig directly
    return appConfig.votingMachineConfig[chainId]?.contractAddress as Address;
  }
  ...
  const data = await Promise.all(
    votingMachineChainIds.map(async (chainId) => {
      ...
      const [oldProposalsData, newProposalsData] = await Promise.all([
        oldFormattedInitialProposals.length > 0
          ...
          : Promise.resolve([]),
        newFormattedInitialProposals.length > 0
          ? votingMachineDataHelper.read.getProposalsData([
              getVotingMachineAddress(
                chainId,
                Number(newFormattedInitialProposals[0].id),
              ),
...
async function parseCache() {
  ...
  if (Object.keys(proposalsCache).length === 0) {
    console.log('No proposals cache found to parse.');
    return;
  }
  ...
  const formattedProposalCache = await Promise.all(
    Object.entries(proposalsCache).map(async (proposal) => {
      const portalContract: any = getContract({
